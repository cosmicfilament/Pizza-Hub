...
const { baseDirCfg, envCfg } = require('./config');
...
server.httpsServerOptions = {
  'key': fs.readFileSync(`${baseDirCfg}/${envCfg.keyFile}`),
  'cert': fs.readFileSync(`${baseDirCfg}/${envCfg.certFile}`)
};
...
server.init = () => {
  ...
  server.httpServer.listen(envCfg.httpPort, () => {
    helpers.log('blue', `The http server is listening on port ${envCfg.httpPort}.`);
    logs.log(`The http server is listening on port ${envCfg.httpPort}.`);
  });

  // Start the HTTPS server
  server.httpsServer.listen(envCfg.httpsPort, () => {
    helpers.log('blue', `The https server is listening on port ${envCfg.httpsPort}.`);
    helpers.log('white', '------------------------------------------');
    logs.log(`The https server is listening on port ${envCfg.httpsPort}.`);
    logs.log('-------------------------------------------');
  });
  ...
  req.on('end', data => {
    ...
    try {
      ...
      handler(requestObj, res)
        .then((responseObj) => {
          helpers.log('green', `Response sent ${responseObj.statusCode}`);
        ...
        .catch((errObj) => {
          ...
          logs.log(`Promise catch handler caught error in server.handler - statusCode: ${status}, Message: ${msg}`);
      ...
      logs.log(`Try/Catch error thrown in server.handler: ${error}`);
