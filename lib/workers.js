...
workers.rotateLogs = async function () {
    ...
    return logs.list(false)
            ...
            if (list) {
                list.forEach(async fileName => {
                    ...
                    if (fileName !== todaysLogs) {
                        ...
                        if (!result) {
                            return helpers.log('red', `Error compressing file: ${newLogId}`);
                        }
                        result = await logs.truncate(logId);
                        if (!result) {
                            return helpers.log('red', `Error truncating file: ${newLogId}`);
                        }
                    }
                });
            }
        ...
        .catch((err) => {
            helpers.log('red', `Rotating logs failed with error code: ${err}`);
        });
};

/**
     * @summary cleanupExpiredTokensLoop
     * @description Currently just purges the expired tokens every 90 minutes
*/
...
/**
     ...
     * @description Compresses yesterday's log file at the start of a new day.
*/
